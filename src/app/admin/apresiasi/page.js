...
export default function AdminApresiasi() {
  ...
  const [expandedPrestasiId, setExpandedPrestasiId] = useState(null); // Accordion State
  ...
  const handleUploadCloudinary = async (file, targetPrestasiId = null) => {
    ...
    try {
      ...
      if (targetPrestasiId && activeEditKader) {
        // Upload untuk spesifik prestasi (Bukti Foto)
        handleUpdatePrestasi(activeEditKader.id, targetPrestasiId, "linkOrFoto", imageUrl);
      } else {
        // Upload untuk Foto Profil Kader Baru
        setNewFoto(imageUrl);
      }
    } catch (error) {
    ...
    reader.onload = (evt) => {
      try {
        ...
        data.forEach(row => {
           ...
           // Tambahkan prestasi ke kader tersebut
  ...
  const handleAddPrestasi = (kaderId, tipe) => {
    ...
    setExpandedPrestasiId(newPrestasiId); // Langsung buka (expand) prestasi yang baru ditambah
  };
  ...
  return (
    <div className="space-y-6 pb-12 max-w-7xl mx-auto px-4">
      ...
      {/* MODAL KELOLA PRESTASI (ACCORDION STYLE) */}
      {activeEditKader && (
                   ...
                   {activeEditKader.prestasi.map((p) => {
                      ...
                      return (
                        <div key={p.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden transition-all duration-200">
                           ...
                           {/* Accordion Body (Form Edit) */}
                           {isExpanded && (
                             <div className="p-4 md:p-5 grid grid-cols-1 md:grid-cols-12 gap-4 bg-white">
                               <div className="md:col-span-12">
                                 <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{isAkad ? 'Judul Jurnal / Karya' : 'Nama Kompetisi / Lomba'}</label>
                                 <input type="text" value={p.judul} onChange={e => handleUpdatePrestasi(activeEditKader.id, p.id, "judul", e.target.value)} className="w-full mt-1 p-2.5 border border-slate-200 bg-slate-50 focus:bg-white rounded-xl text-sm outline-none focus:border-blue-500 transition" placeholder="Tulis di sini..."/>
                               </div>
                               
                               <div className="md:col-span-6">
                                 <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{isAkad ? 'Pencapaian (Misal: Sinta 2, Penulis ke-1)' : 'Pencapaian (Misal: Juara 1, Finalis)'}</label>
                                 <input type="text" value={p.pencapaian} onChange={e => handleUpdatePrestasi(activeEditKader.id, p.id, "pencapaian", e.target.value)} className="w-full mt-1 p-2.5 border border-slate-200 bg-slate-50 focus:bg-white rounded-xl text-sm outline-none focus:border-blue-500 transition" placeholder="Tulis di sini..."/>
                               </div>

                               <div className="md:col-span-3 col-span-2">
                                 <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Tingkat</label>
                                 <select value={p.tingkat} onChange={e => handleUpdatePrestasi(activeEditKader.id, p.id, "tingkat", e.target.value)} className="w-full mt-1 p-2.5 border border-slate-200 bg-slate-50 focus:bg-white rounded-xl text-sm outline-none focus:border-blue-500 transition appearance-none">
                               ...
                               <div className="md:col-span-3 col-span-2">
                                 ...
                                 <input type="number" value={p.tahun} onChange={e => handleUpdatePrestasi(activeEditKader.id, p.id, "tahun", e.target.value)} className="w-full mt-1 p-2.5 border border-slate-200 bg-slate-50 focus:bg-white rounded-xl text-sm outline-none focus:border-blue-500 transition" placeholder="Misal: 2024"/>
                               </div>
                               ...
                               <div className="md:col-span-12 mt-2">
                                 ...
                                 <div className="flex gap-2">
                                   ...
                                   {!isAkad && (
                                     <label className="bg-slate-100 hover:bg-slate-200 cursor-pointer px-4 border border-slate-200 rounded-xl flex items-center justify-center transition">
