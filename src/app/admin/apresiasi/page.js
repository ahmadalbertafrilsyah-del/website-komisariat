...
export default function AdminApresiasi() {
  ...
  return (
    <div className="space-y-6 pb-12 w-full text-sm">
      ...
      {/* FORM TAMBAH KADER BARU (DIBUAT COLLAPSEABLE AGAR TIDAK RIBET) */}
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm mb-6">
        ...
        >
          <span className="flex items-center gap-2"><Plus size={18} className="text-blue-600" /> Tambah Data Kader Berprestasi</span>
        ...
        {isFormOpen && (
          <div className="p-6 border-t border-slate-200 animate-in slide-in-from-top-2">
            ...
            <div className="mb-6 bg-blue-50 border border-blue-100 rounded-md p-4 text-xs text-blue-800 flex gap-2">
              ...
              <p><strong>Panduan Cepat:</strong> Masukkan detail profil kader baru melalui form ini terlebih dahulu. Setelah data kader terbuat, Anda bisa mengedit dan menambahkan riwayat prestasi/jurnal mereka dari daftar tabel di bawah secara praktis.</p>
            </div>
      ...
      {/* Grid Utama (Kiri: List Kader, Kanan: Form Edit Prestasi) */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        
        {/* KOLOM KIRI: DAFTAR KADER */}
        <div className="lg:col-span-5 flex flex-col gap-4">
           <div className="relative w-full">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400"/>
              <input type="text" placeholder="Cari kader / judul prestasi..." value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} className={inputStandardClass + " pl-9 bg-white"}/>
           </div>

           <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden flex flex-col max-h-[600px]">
              <div className="p-4 border-b border-slate-200 bg-slate-50">
                 <h3 className="font-semibold text-slate-800 flex items-center gap-2"><Trophy size={16} className="text-slate-500" /> Database Kader Terpilih</h3>
              </div>
              <div className="overflow-y-auto p-4 space-y-3 hide-scrollbar">
                {filteredData.length === 0 ? (
                  <div className="text-center py-10 text-slate-500 font-medium">Data apresiasi tidak ditemukan.</div>
                ) : (
                  filteredData.map(k => (
                    <div 
                      key={k.id} 
                      onClick={() => setActiveEditKader(k)}
                      className={`border p-3 rounded-lg flex items-center gap-3 cursor-pointer transition-colors ${activeEditKader?.id === k.id ? 'border-blue-500 bg-blue-50' : 'border-slate-200 bg-white hover:border-slate-300'}`}
                    >
                       <div className="w-12 h-12 rounded-md overflow-hidden bg-slate-100 border border-slate-200 shrink-0 flex items-center justify-center">
                         {k.fotoKader ? <img src={k.fotoKader} className="w-full h-full object-cover"/> : <ImageIcon size={20} className="text-slate-300"/>}
                       </div>
                       <div className="flex-1 min-w-0">
                          <h4 className="font-bold text-slate-800 text-sm truncate">{k.namaLengkap}</h4>
                          <p className="text-[11px] text-slate-500 truncate">{k.asalRayon}</p>
                       </div>
                       <div className="shrink-0 bg-white px-2 py-1 rounded border border-slate-200 text-[10px] font-bold text-slate-500 flex items-center gap-1 shadow-sm">
                         {k.prestasi.length} <Trophy size={10}/>
                       </div>
                    </div>
                  ))
                )}
        ...
        {/* KOLOM KANAN: DETAIL & EDIT PRESTASI */}
        <div className="lg:col-span-7">
           {activeEditKader ? (
             <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden h-full flex flex-col">
                <div className="p-4 md:p-6 border-b border-slate-200 flex justify-between items-start bg-slate-50">
                   <div>
                     <h2 className="text-lg font-bold text-slate-900 mb-1">Riwayat: {activeEditKader.namaLengkap}</h2>
                     <p className="text-xs text-slate-500">{activeEditKader.asalRayon}</p>
                   </div>
                   <button onClick={() => handleDeleteKader(activeEditKader.id)} className="text-red-500 hover:bg-red-50 p-2 rounded-md transition-colors border border-transparent hover:border-red-200 flex items-center gap-1.5 text-xs font-semibold">
                      <Trash2 size={14}/> Hapus Kader
                   </button>
                </div>

                <div className="p-4 md:p-6 flex gap-3 border-b border-slate-100">
                   <button onClick={() => handleAddPrestasi(activeEditKader.id, "akademik")} className="flex-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 font-semibold py-2 px-3 rounded-md text-xs flex justify-center items-center gap-1.5 transition">
                     <Plus size={14}/> Tambah Akademik
                   </button>
                   <button onClick={() => handleAddPrestasi(activeEditKader.id, "non-akademik")} className="flex-1 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 font-semibold py-2 px-3 rounded-md text-xs flex justify-center items-center gap-1.5 transition">
                     <Plus size={14}/> Tambah Lomba/Non-Akad
                   </button>
                </div>

                <div className="p-4 md:p-6 overflow-y-auto space-y-4 max-h-[500px] hide-scrollbar">
                   {activeEditKader.prestasi.length === 0 ? (
                      <div className="text-center py-10 text-slate-400">Belum ada riwayat prestasi ditambahkan untuk kader ini.</div>
                   ) : (
                      activeEditKader.prestasi.map((p) => (
                        <div key={p.id} className="border border-slate-200 rounded-lg overflow-hidden">
                           <div 
                             onClick={() => setExpandedPrestasiId(expandedPrestasiId === p.id ? null : p.id)}
                             className={`px-4 py-3 flex justify-between items-center cursor-pointer transition-colors ${expandedPrestasiId === p.id ? 'bg-blue-50 border-b border-blue-100' : 'bg-slate-50 hover:bg-slate-100'}`}
                           >
                              <div className="flex items-center gap-2 overflow-hidden pr-3">
                                {p.tipe.toLowerCase().includes("akad") ? <BookOpen size={16} className="text-emerald-600 shrink-0"/> : <Award size={16} className="text-amber-600 shrink-0"/>}
                                <span className="font-semibold text-slate-800 text-sm truncate">{p.judul || "Prestasi Baru (Belum ada judul)"}</span>
                              </div>
                              <div className="flex items-center gap-3 shrink-0">
                                <button onClick={(e) => { e.stopPropagation(); handleDeletePrestasi(activeEditKader.id, p.id); }} className="text-slate-400 hover:text-red-500">
                                   <Trash2 size={16}/>
                                </button>
                                {expandedPrestasiId === p.id ? <ChevronUp size={16} className="text-slate-500"/> : <ChevronDown size={16} className="text-slate-500"/>}
                              </div>
                           </div>
                           
                           {expandedPrestasiId === p.id && (
                             <div className="p-4 space-y-4 animate-in slide-in-from-top-2">
                               <div className="grid grid-cols-2 gap-4">
                                 <div className="col-span-2">
                                   <label className={labelStandardClass}>Judul Jurnal / Nama Kompetisi</label>
                                   <input type="text" value={p.judul} onChange={e => handleUpdatePrestasi(activeEditKader.id, p.id, "judul", e.target.value)} className={inputStandardClass} placeholder="Contoh: Juara 1 Lomba Essai Nasional..."/>
                                 </div>
                                 <div className="col-span-2 md:col-span-1">
                                   <label className={labelStandardClass}>Pencapaian / Keterangan Singkat</label>
                                   <input type="text" value={p.pencapaian} onChange={e => handleUpdatePrestasi(activeEditKader.id, p.id, "pencapaian", e.target.value)} className={inputStandardClass} placeholder="Juara 1 / Sinta 2 / Penulis Terbaik"/>
                                 </div>
                                 <div className="col-span-2 md:col-span-1">
                                   <label className={labelStandardClass}>Tingkat / Jenjang</label>
                                   <input type="text" value={p.tingkat} onChange={e => handleUpdatePrestasi(activeEditKader.id, p.id, "tingkat", e.target.value)} className={inputStandardClass} placeholder="Regional / Nasional / Fakultas"/>
                                 </div>
                                 <div className="col-span-2 md:col-span-1">
                                   <label className={labelStandardClass}>Tahun / Angkatan</label>
                                   <input type="text" value={p.tahun} onChange={e => handleUpdatePrestasi(activeEditKader.id, p.id, "tahun", e.target.value)} className={inputStandardClass} placeholder="2025"/>
                                 </div>
                                 <div className="col-span-2 md:col-span-1">
                                   <label className={labelStandardClass}>Kategori Tipe (Internal)</label>
                                   <select value={p.tipe} onChange={e => handleUpdatePrestasi(activeEditKader.id, p.id, "tipe", e.target.value)} className={inputStandardClass}>
                                      <option value="akademik">Akademik / Kaderisasi</option>
                                      <option value="non-akademik">Lomba / Non-Akademik</option>
                                   </select>
                                 </div>
                                 <div className="col-span-2">
                                   <label className={labelStandardClass}>Bukti Link (G-Drive / Jurnal) ATAU Upload Foto</label>
                                   <div className="flex gap-2">
                                     <input type="text" value={p.linkOrFoto} onChange={e => handleUpdatePrestasi(activeEditKader.id, p.id, "linkOrFoto", e.target.value)} className={`${inputStandardClass} flex-1 font-mono text-[11px]`} placeholder="https://..."/>
                                     <label className="bg-slate-50 hover:bg-slate-100 text-slate-600 cursor-pointer px-3 py-2 border border-slate-300 rounded-md flex items-center justify-center transition-colors shadow-sm shrink-0">
             ...
             <div className="bg-white rounded-lg shadow-sm border border-slate-200 h-full flex flex-col items-center justify-center p-10 text-center min-h-[400px]">
                <div className="w-16 h-16 bg-slate-50 border border-slate-100 rounded-full flex items-center justify-center mb-4">
                  <Award size={32} className="text-slate-300"/>
                </div>
                <h3 className="font-bold text-slate-700 text-lg">Pilih Kader untuk Mengedit Prestasi</h3>
                <p className="text-slate-500 mt-2 text-sm max-w-sm">Klik salah satu nama kader di panel sebelah kiri untuk melihat dan menambahkan riwayat apresiasi mereka.</p>
             </div>
           )}
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-lg border border-slate-200 sticky bottom-6 z-40 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)]">
         <div className="flex items-start gap-2.5 text-sm text-slate-600 max-w-2xl">
           ...
           <p>Pastikan Anda selalu menekan tombol simpan agar penambahan profil maupun riwayat apresiasi kader diperbarui ke sistem publik.</p>
         </div>
