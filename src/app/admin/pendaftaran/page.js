...
import { 
  ClipboardList, Users, Plus, Trash2, Edit, Save, Download, Settings, 
  UploadCloud, Loader2, Image as ImageIcon, XCircle, Copy, Eye, X, 
  Share2, ExternalLink, Link as LinkIcon, Search, CheckSquare, CreditCard, Clock, Calendar, ArrowUp, ArrowDown
} from "lucide-react";
...
export default function AdminPendaftaran() {
  ...
  const defaultPaymentConfig = {
    isRequired: false,
    categories: [{ id: Date.now().toString(), name: "Tiket Umum", price: "0" }],
    methods: "Transfer Bank BCA: 123456789 a.n PMII\nDANA: 08123456789",
    requireProof: true
  };
  ...
  const [formData, setFormData] = useState({
    ...
    kuota: "", deadline: "", pesanSukses: "Terima kasih, pendaftaran Anda berhasil direkam!", 
  ...
  // ================= BUILDER PEMBAYARAN (HTM) =================
  const togglePaymentStatus = () => {
    setFormData(prev => ({ ...prev, payment: { ...prev.payment, isRequired: !prev.payment.isRequired } }));
  };
  const addPaymentCategory = () => {
    setFormData(prev => ({ ...prev, payment: { ...prev.payment, categories: [...prev.payment.categories, { id: Date.now().toString() + Math.random(), name: "", price: "" }] } }));
  };
  const updatePaymentCategory = (id, field, value) => {
    setFormData(prev => ({ ...prev, payment: { ...prev.payment, categories: prev.payment.categories.map(c => c.id === id ? { ...c, [field]: value } : c) } }));
  };
  const removePaymentCategory = (id) => {
    setFormData(prev => ({ ...prev, payment: { ...prev.payment, categories: prev.payment.categories.filter(c => c.id !== id) } }));
  };
  ...
  const addCustomQuestion = (type) => {
    ...
    setFormData(prev => ({ 
      ...
      customQuestions: [...prev.customQuestions, { 
        id: uniqueId, type, question: "", description: "", 
        options: (type === 'select' || type === 'radio' || type === 'checkbox') ? ["Opsi 1"] : [], 
        required: true 
      }] 
    }));
  };
  ...
  // --- FUNGSI PINDAH URUTAN PERTANYAAN ---
  ...
  const moveQuestionDown = (index) => {
    if (index === formData.customQuestions.length - 1) return;
    setFormData(prev => {
      const newQs = [...prev.customQuestions];
      const temp = newQs[index + 1];
      newQs[index + 1] = newQs[index];
  ...
  const handleSaveFormulir = async (e) => {
    ...
    if (formData.payment.isRequired) {
      const invalidPayment = formData.payment.categories.find(c => !c.name.trim() || !c.price.trim());
      if (invalidPayment) return alert("Terdapat kategori harga yang belum diisi nama atau nominalnya!");
    }
  ...
  const handleEditClick = (form) => {
    ...
    setFormData({ 
      ...form, 
      payment: form.payment || defaultPaymentConfig,
  ...
  const handleCloneForm = (form) => {
    ...
    setFormData({ 
      ...form, judul: `${form.judul} (Salinan)`, status: "Tutup",
      payment: form.payment || defaultPaymentConfig
    });
    window.scrollTo({ top: 0, behavior: 'smooth' });
  ...
  const resetFormState = () => {
    ...
    setFormData({ 
      judul: "", kategori: "", deskripsi: "", status: "Buka", linkGrupWA: "", thumbnailUrl: "", 
      kuota: "", deadline: "", pesanSukses: "Terima kasih, pendaftaran Anda berhasil direkam!", 
      payment: defaultPaymentConfig, customQuestions: [] 
    });
  };
    ...
    const formattedData = dataToExport.map((p, index) => {
      ...
      if (p.answers) {
        ...
        if (originalForm && originalForm.customQuestions) {
          originalForm.customQuestions.forEach(q => { if (p.answers[q.question] !== undefined) baseData[q.question] = Array.isArray(p.answers[q.question]) ? p.answers[q.question].join(", ") : p.answers[q.question]; });
          Object.keys(p.answers).forEach(qTitle => { if(baseData[qTitle] === undefined) baseData[qTitle] = Array.isArray(p.answers[qTitle]) ? p.answers[qTitle].join(", ") : p.answers[qTitle]; });
        } else { Object.keys(p.answers).forEach(qTitle => baseData[qTitle] = Array.isArray(p.answers[qTitle]) ? p.answers[qTitle].join(", ") : p.answers[qTitle]); }
      }
      if (p.paymentProof) baseData["Bukti Pembayaran"] = p.paymentProof;
      if (p.paymentCategory) baseData["Kategori Tiket"] = p.paymentCategory;
      return baseData;
    });
      ...
      if (originalForm && originalForm.customQuestions) {
          originalForm.customQuestions.forEach(q => {
              if (pendaftar.answers[q.question] !== undefined) { 
                let ans = pendaftar.answers[q.question];
                ordered.push({ pertanyaan: q.question, jawaban: Array.isArray(ans) ? ans.join(", ") : ans }); 
                keysSudahDiproses.add(q.question); 
              }
          });
      }
      Object.entries(pendaftar.answers).forEach(([q, a]) => { if (!keysSudahDiproses.has(q)) ordered.push({ pertanyaan: q, jawaban: Array.isArray(a) ? a.join(", ") : a }); });
      return ordered;
  }
  ...
  return (
    <div className="space-y-6 pb-12 w-full text-sm">
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">Kelola Pendaftaran</h1>
          <p className="text-sm text-slate-500 mt-1">Buat formulir kegiatan, atur biaya tiket, dan kelola pendaftar secara otomatis.</p>
        </div>
      </div>
          ...
          <div className="bg-white p-5 md:p-6 rounded-lg border border-slate-200 shadow-sm lg:col-span-2">
            ...
            <form onSubmit={handleSaveFormulir} className="space-y-6">
              ...
              <div className="bg-slate-50/50 p-5 rounded-lg border border-slate-200 space-y-4">
                <h3 className="font-bold text-slate-700 border-b border-slate-200 pb-2 mb-4">1. Informasi Dasar</h3>
                <div className="grid md:grid-cols-3 gap-4">
                  <div className="md:col-span-2">
                    <label className={labelStandardClass}>Judul Pendaftaran</label>
                    <input type="text" required value={formData.judul} onChange={(e) => setFormData({...formData, judul: e.target.value})} placeholder="Pendaftaran Seminar Nasional..." className={inputStandardClass} />
                  </div>
                  <div>
                    <label className={labelStandardClass}>Kategori / Tingkatan</label>
                    <input type="text" required value={formData.kategori} onChange={(e) => setFormData({...formData, kategori: e.target.value})} placeholder="Seminar..." className={inputStandardClass} />
                  </div>
                </div>
                <div>
                  <label className={labelStandardClass}>Deskripsi Lengkap / Persyaratan</label>
                  <textarea rows="3" value={formData.deskripsi} onChange={(e) => setFormData({...formData, deskripsi: e.target.value})} placeholder="Jelaskan instruksi pengisian form ini..." className={inputStandardClass} />
                </div>
                <div className="grid md:grid-cols-2 gap-4 items-end">
                   <div>
                      <label className={labelStandardClass}>Thumbnail / Pamflet (Opsional)</label>
                   ...
                   <div>
                    <label className={labelStandardClass}>Link Grup WA (Bila Sukses Daftar)</label>
              ...
              {/* BAGIAN 2: PEMBAYARAN (HTM) */}
              <div className={`p-5 rounded-lg border transition-colors ${formData.payment.isRequired ? 'bg-indigo-50/30 border-indigo-200' : 'bg-slate-50/50 border-slate-200'}`}>
                 <div className="flex items-center justify-between mb-4">
                    <h3 className="font-bold text-slate-700 flex items-center gap-2">
                      <CreditCard size={18} className={formData.payment.isRequired ? "text-indigo-600" : "text-slate-400"}/> 2. Pengaturan Biaya & Pembayaran (HTM)
                    </h3>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <span className="text-xs font-semibold text-slate-600">Aktifkan Fitur Berbayar</span>
                      <div className={`w-10 h-5 flex items-center rounded-full p-1 transition-colors ${formData.payment.isRequired ? 'bg-indigo-600' : 'bg-slate-300'}`}>
                        <input type="checkbox" className="hidden" checked={formData.payment.isRequired} onChange={togglePaymentStatus} />
                        <div className={`bg-white w-3.5 h-3.5 rounded-full shadow-md transform transition-transform ${formData.payment.isRequired ? 'translate-x-5' : ''}`}></div>
                      </div>
                    </label>
                 </div>

                 {formData.payment.isRequired && (
                   <div className="space-y-4 mt-4 border-t border-indigo-100 pt-4 animate-in fade-in">
                      <div className="space-y-3">
                        <label className={labelStandardClass}>Kategori & Nominal Biaya (Bisa Lebih dari 1)</label>
                        {formData.payment.categories.map((cat, idx) => (
                           <div key={cat.id} className="flex gap-2 items-center">
                             <input type="text" placeholder="Nama Kategori (Cth: Kader PMII, Umum)" value={cat.name} onChange={(e) => updatePaymentCategory(cat.id, 'name', e.target.value)} className={inputStandardClass} />
                             <input type="number" placeholder="Nominal (Cth: 50000)" value={cat.price} onChange={(e) => updatePaymentCategory(cat.id, 'price', e.target.value)} className={`${inputStandardClass} w-32 md:w-48`} />
                             {formData.payment.categories.length > 1 && (
                               <button type="button" onClick={() => removePaymentCategory(cat.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-md transition"><Trash2 size={16}/></button>
                             )}
                           </div>
                        ))}
                        <button type="button" onClick={addPaymentCategory} className="text-xs font-semibold text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-md hover:bg-indigo-100 transition w-max">+ Tambah Kategori Harga</button>
                      </div>

                      <div className="grid md:grid-cols-2 gap-4 pt-2">
                        <div>
                           <label className={labelStandardClass}>Instruksi / Metode Pembayaran</label>
                           <textarea rows="3" value={formData.payment.methods} onChange={(e) => setFormData({...formData, payment: {...formData.payment, methods: e.target.value}})} className={inputStandardClass} placeholder="No. Rekening, e-Wallet, atau kontak Bendahara..." />
                        </div>
                        <div className="flex flex-col justify-end pb-2 space-y-3">
                           <label className="flex items-center gap-2 cursor-pointer p-3 bg-white border border-indigo-100 rounded-md shadow-sm">
                             <input type="checkbox" checked={formData.payment.requireProof} onChange={(e) => setFormData({...formData, payment: {...formData.payment, requireProof: e.target.checked}})} className="rounded text-indigo-600 focus:ring-indigo-500 w-4 h-4" />
                             <span className="text-sm font-semibold text-slate-700">Wajibkan Upload Bukti Transfer (Otomatis dibuatkan kolom upload)</span>
                           </label>
                        </div>
                      </div>
                   </div>
              ...
              <div className="bg-slate-50/50 p-5 rounded-lg border border-slate-200 space-y-4">
                 <h3 className="font-bold text-slate-700 border-b border-slate-200 pb-2 mb-4">3. Pengaturan Validasi</h3>
                 ...
                 <div>
                    <label className={labelStandardClass}>Pesan Sukses (Muncul setelah klik Kirim)</label>
              ...
              {/* BAGIAN 4: BUILDER PERTANYAAN STYLE GOOGLE FORM */}
              <div className="border border-slate-200 rounded-lg overflow-hidden bg-slate-50/30">
                 <div className="bg-blue-600 p-4 flex justify-between items-center text-sm font-bold text-white shadow-sm">
                    4. Desain Formulir
                 </div>
                 <div className="p-4 md:p-6 space-y-6">
                    {formData.customQuestions.map((q, index) => (
                      <div key={q.id} className="bg-white p-5 md:p-6 border border-slate-200 rounded-xl shadow-sm relative group focus-within:border-blue-400 focus-within:shadow-md transition-all">
                         
                         {/* Header Pertanyaan & Dropdown Tipe */}
                         <div className="flex flex-col md:flex-row justify-between gap-4 items-start mb-4">
                            <div className="w-full md:w-2/3 space-y-2">
                               <input 
                                 ...
                                 className="w-full p-2.5 bg-slate-50/50 border-b-2 border-slate-300 focus:border-blue-500 outline-none font-bold text-base transition-colors rounded-t-md"
                               />
                               <input 
                                 type="text" value={q.description || ''} onChange={(e) => updateQuestion(q.id, 'description', e.target.value)}
                                 placeholder="Deskripsi pertanyaan (opsional)" 
                                 className="w-full px-2.5 py-1.5 text-xs text-slate-500 bg-transparent border-b border-transparent focus:border-slate-300 outline-none"
                               />
                            </div>
                            <select value={q.type} onChange={(e) => updateQuestion(q.id, 'type', e.target.value)} className="w-full md:w-auto p-2.5 text-sm bg-white border border-slate-300 rounded-md outline-none shrink-0 cursor-pointer font-medium shadow-sm">
                               ...
                               <option value="select">Dropdown (Menu Tarik-Turun)</option>
                         ...
                         {/* Render Input Dinamis Sesuai Tipe */}
                             ...
                             <div className="space-y-3 p-3 bg-slate-50/50 rounded-lg border border-slate-100">
                               {q.options.map((opt, oIdx) => (
                                 <div key={oIdx} className="flex items-center gap-3">
                                   <div className={`w-4 h-4 border-2 border-slate-400 shrink-0 ${q.type === 'radio' ? 'rounded-full' : 'rounded-sm'}`}></div>
                                   <input type="text" required value={opt} onChange={(e) => updateOption(q.id, oIdx, e.target.value)} className="text-sm border-b border-slate-300 bg-transparent outline-none flex-grow pb-1 focus:border-blue-500 font-medium" placeholder={`Opsi ${oIdx + 1}`} />
                                   {q.options.length > 1 && <button type="button" onClick={() => removeOption(q.id, oIdx)} className="text-slate-400 hover:text-red-500"><X size={18}/></button>}
                                 </div>
                               ))}
                               <button type="button" onClick={() => addOption(q.id)} className="text-sm font-semibold text-blue-600 mt-2 hover:underline flex items-center gap-1 w-max">Tambahkan Opsi</button>
                             </div>
                           )}

                           {/* Tipe Teks Placeholder */}
                           {q.type === 'text' && <div className="border-b border-slate-300 border-dotted pb-1 text-slate-400 text-sm w-1/2">Teks jawaban singkat</div>}
                           {q.type === 'textarea' && <div className="border-b border-slate-300 border-dotted pb-1 text-slate-400 text-sm w-3/4">Teks jawaban panjang</div>}
                           
                           {/* Tipe Tambahan Baru */}
                           {q.type === 'file' && <div className="flex items-center gap-2 text-sm text-slate-500 bg-slate-100 px-3 py-2 rounded-md w-max"><UploadCloud size={16}/> Responden dapat mengunggah file</div>}
                           {q.type === 'date' && <div className="flex items-center gap-2 text-sm text-slate-500 bg-slate-100 px-3 py-2 rounded-md w-max"><Calendar size={16}/> Bulan, hari, tahun</div>}
                           {q.type === 'time' && <div className="flex items-center gap-2 text-sm text-slate-500 bg-slate-100 px-3 py-2 rounded-md w-max"><Clock size={16}/> Waktu</div>}
                         </div>

                         {/* Footer Pertanyaan */}
                         <div className="flex justify-end items-center gap-3 sm:gap-5 mt-6 pt-4 border-t border-slate-100">
                           {/* Tombol Pindah Urutan */}
                           <div className="flex items-center gap-1 mr-auto">
                             <button type="button" onClick={() => moveQuestionUp(index)} disabled={index === 0} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded disabled:opacity-30 disabled:cursor-not-allowed transition tooltip" title="Geser ke Atas"><ArrowUp size={18}/></button>
                             <button type="button" onClick={() => moveQuestionDown(index)} disabled={index === formData.customQuestions.length - 1} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded disabled:opacity-30 disabled:cursor-not-allowed transition tooltip" title="Geser ke Bawah"><ArrowDown size={18}/></button>
                           </div>

                           <button type="button" onClick={() => {
                             const clone = {...q, id: Date.now().toString()};
                             setFormData(prev => ({...prev, customQuestions: [...prev.customQuestions, clone]}));
                           }} className="text-slate-500 hover:text-slate-800 transition tooltip" title="Duplikasikan"><Copy size={18}/></button>
                           <button type="button" onClick={() => deleteQuestion(q.id)} className="text-slate-500 hover:text-red-600 transition tooltip" title="Hapus"><Trash2 size={18}/></button>
                           <div className="w-px h-6 bg-slate-300"></div>
                           <label className="text-sm font-semibold text-slate-700 flex items-center gap-2 cursor-pointer">
                             Wajib diisi 
                             <div className={`w-9 h-5 flex items-center rounded-full p-1 transition-colors ${q.required ? 'bg-blue-600' : 'bg-slate-300'}`}>
                               <input type="checkbox" className="hidden" checked={q.required} onChange={(e) => updateQuestion(q.id, 'required', e.target.checked)} />
                               <div className={`bg-white w-3.5 h-3.5 rounded-full shadow-md transform transition-transform ${q.required ? 'translate-x-3.5' : ''}`}></div>
                             </div>
                           </label>
                         </div>
                      </div>
                    ...
                    {/* Floating Toolbar Tambah Item */}
                    <div className="sticky bottom-4 flex justify-center w-full z-10 pt-4">
                       <div className="bg-white border border-slate-200 shadow-xl rounded-full p-2 flex items-center gap-1">
                          <button type="button" onClick={() => addCustomQuestion('text')} className="flex items-center gap-1.5 hover:bg-slate-100 text-slate-700 font-semibold text-xs px-3 py-2 rounded-full transition"><Plus size={14}/> Tambah Pertanyaan</button>
                          <div className="w-px h-5 bg-slate-200 mx-1"></div>
                          <button type="button" onClick={() => addCustomQuestion('radio')} className="hover:bg-slate-100 text-slate-600 p-2 rounded-full transition tooltip" title="Pilihan Ganda"><CheckSquare size={16}/></button>
                          <button type="button" onClick={() => addCustomQuestion('file')} className="hover:bg-slate-100 text-slate-600 p-2 rounded-full transition tooltip" title="Upload File"><UploadCloud size={16}/></button>
                       </div>
                    </div>
                 </div>
              </div>
              ...
              <div className="flex justify-end pt-4">
                 <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 px-8 rounded-lg shadow-md transition flex items-center gap-2 w-full sm:w-auto justify-center text-base">
                   <Save size={18} /> {isEditingForm ? "Simpan Perubahan Formulir" : "Terbitkan Formulir Baru"}
                 </button>
              </div>
            </form>
          </div>
             ...
             {formulirList.length === 0 ? (
                ...
                formulirList.map((form) => (
                  <div key={form.id} className={`bg-white rounded-lg border transition-colors p-5 relative shadow-sm ${editFormId === form.id ? 'border-amber-400' : 'border-slate-200 hover:border-blue-300'}`}>
                    ...
                    <h3 className="font-bold text-slate-800 text-sm leading-snug mb-1">{form.judul}</h3>
                    {form.payment?.isRequired && <p className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded w-max mb-2">HTM / Berbayar</p>}
                    
                    <div className="text-xs text-slate-500 flex items-center gap-1.5 mb-4 mt-2"><Users size={14}/> {pendaftarList.filter(p => p.formId === form.id).length} Data masuk</div>
      ...
      {/* BLOK TABS PENDAFTAR (SAMA SEPERTI SEBELUMNYA) */}
      {activeTab === "pendaftar" && (
      ...
      {/* MODAL DETAIL PENDAFTAR (TAMBAH TAMPILAN BUKTI BAYAR) */}
      {viewDetailModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg relative z-10 overflow-hidden flex flex-col max-h-[85vh] border border-slate-200">
            ...
            <div className="p-6 overflow-y-auto space-y-5 bg-white">
               ...
               {viewDetailModal.paymentCategory && (
                 <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-lg mb-4">
                    <h4 className="text-xs font-bold text-indigo-800 uppercase mb-2 flex items-center gap-1.5"><CreditCard size={14}/> Info Pembayaran</h4>
                    <div className="text-sm font-semibold text-slate-800">Kategori: <span className="text-indigo-600">{viewDetailModal.paymentCategory}</span></div>
                    {viewDetailModal.paymentProof && (
                      <a href={viewDetailModal.paymentProof} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex items-center gap-1.5 bg-white text-blue-600 hover:bg-blue-50 px-3 py-1.5 rounded-md text-xs font-bold transition border border-blue-200 shadow-sm"><ExternalLink size={14}/> Lihat Bukti Transfer</a>
                    )}
                 </div>
               )}
                 ...
                 getOrderedAnswers(viewDetailModal).map((item, idx) => {
                   ...
                   return (
                     <div key={uniqueKey} className="border-b border-slate-100 pb-4">
                        <label className="text-xs font-semibold text-slate-500 block mb-1.5">{item.pertanyaan}</label>
