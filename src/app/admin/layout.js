...
export default function AdminLayout({ children }) {
  ...
  return (
    <div className="flex h-screen w-full bg-slate-50 overflow-hidden font-sans text-slate-800">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#0f172a] text-white flex flex-col transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${sidebarOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full"}`}>
        <div className="p-6 border-b border-slate-800 flex items-center justify-between md:justify-center">
          ...
          <button className="md:hidden text-slate-400 hover:text-white bg-slate-800 p-1.5 rounded-lg" onClick={() => setSidebarOpen(false)}><X size={18} /></button>
        </div>
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto hide-scrollbar">
          ...
          <Link href="/admin/alumni" onClick={() => setSidebarOpen(false)} className={menuStyle("/admin/alumni")}><GraduationCap size={18} className={pathname === "/admin/alumni" ? "text-blue-400" : ""} /> Kelola Alumni</Link>
        ...
        <div className="p-4 border-t border-slate-800">
          <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 bg-red-500/10 hover:bg-red-500 hover:text-white text-red-400 p-3 rounded-xl font-bold transition-colors duration-200">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        <header className="md:hidden bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between sticky top-0 z-30 shadow-sm">
          <div className="flex items-center gap-3">
            <button className="p-2 bg-slate-50 border border-slate-200 text-slate-700 rounded-lg hover:bg-slate-100 transition-colors" onClick={() => setSidebarOpen(true)}>
              <Menu size={20} />
            </button>
            <h1 className="font-bold text-slate-800 text-sm">Dashboard</h1>
          </div>
          <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center font-bold text-xs text-white">P</div>
        </header>
